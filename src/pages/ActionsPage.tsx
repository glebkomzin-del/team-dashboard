...
interface ActionsPageProps {
  ...
  cycleBlocker: (b: Blocker) => void
  cycleOpen: (o: OpenItem) => void
                  ...
                  <Card className="glass-card border-[var(--syn-line)]"><CardContent data-testid="todos-table-scroll" className="p-0 max-h-[calc(100vh-196px)] overflow-y-auto"><Table className="table-fixed w-full"><TableHeader><TableRow className="border-[var(--syn-line)]">
                    ...
                    {filteredTodos.map(t => { const overdue = t.dueDate && t.dueDate < today && t.status !== 'done'; return (
                      <TableRow key={t.id} className={`${TABLE_ROW_CLASS} text-sm border-[var(--syn-line)] group select-none ${t.status === 'done' ? 'opacity-40' : ''} ${todoSelected.has(t.id) ? 'bg-[var(--syn-accent)]/5' : 'hover:bg-[var(--syn-hover)]'}`} onClick={() => setTodoSelected(prev => { const n = new Set(prev); n.has(t.id) ? n.delete(t.id) : n.add(t.id); return n })}>
                        <TableCell className="text-left py-1"><div className="flex items-center gap-2"><StatusCycleButton status={t.status} type="todo" onClick={() => cycleTodo(t)} /><div className={TITLE_WRAP_CLASS}><button onClick={e => { e.stopPropagation(); setViewTodo(t) }} className={`text-left font-normal hover:text-[var(--syn-accent)] truncate max-w-full ${t.status === 'done' ? 'line-through' : ''}`}>{t.title}</button>{t.description && <div className="text-xs truncate max-w-sm" style={{ color: 'var(--syn-text-faint)' }}>{t.description}</div>}</div></div></TableCell>
                  ...
                  <Card className="glass-card border-[var(--syn-line)]"><CardContent data-testid="blockers-table-scroll" className="p-0 max-h-[calc(100vh-196px)] overflow-y-auto"><Table className="table-fixed w-full"><TableHeader><TableRow className="border-[var(--syn-line)]">
                    ...
                    {filteredBlockers.map(b => (
                      <TableRow key={b.id} className={`${TABLE_ROW_CLASS} text-sm border-[var(--syn-line)] group select-none cursor-pointer ${b.status !== 'active' ? 'opacity-50' : ''} ${blockerSelected.has(b.id) ? 'bg-[var(--syn-accent)]/5' : 'hover:bg-[var(--syn-hover)]'}`} onClick={() => setBlockerSelected(prev => { const n = new Set(prev); n.has(b.id) ? n.delete(b.id) : n.add(b.id); return n })}>
                        <TableCell className="text-left py-1"><div className="flex items-center gap-2"><StatusCycleButton status={b.status} type="blocker" onClick={() => cycleBlocker(b)} /><div className={TITLE_WRAP_CLASS}><button onClick={e => { e.stopPropagation(); setViewBlocker(b) }} className="text-left font-normal hover:text-[var(--syn-accent)] truncate max-w-full">{b.title}</button><div className="text-xs truncate max-w-md" style={{ color: 'var(--syn-text-faint)' }}>{b.description}</div></div></div></TableCell>
                  ...
                  <Card className="glass-card border-[var(--syn-line)]"><CardContent data-testid="open-items-table-scroll" className="p-0 max-h-[calc(100vh-196px)] overflow-y-auto"><Table className="table-fixed w-full"><TableHeader><TableRow className="border-[var(--syn-line)]">
                    ...
                    {filteredOpen.map(o => (
                      <TableRow key={o.id} className={`${TABLE_ROW_CLASS} text-sm border-[var(--syn-line)] group select-none cursor-pointer ${o.status === 'closed' ? 'opacity-40' : ''} ${openSelected.has(o.id) ? 'bg-[var(--syn-accent)]/5' : 'hover:bg-[var(--syn-hover)]'}`} onClick={() => setOpenSelected(prev => { const n = new Set(prev); n.has(o.id) ? n.delete(o.id) : n.add(o.id); return n })}>
                        <TableCell className="text-left py-1"><div className="flex items-center gap-2"><StatusCycleButton status={o.status} type="open_item" onClick={() => cycleOpen(o)} /><div className={TITLE_WRAP_CLASS}><button onClick={e => { e.stopPropagation(); setViewOpen(o) }} className="text-left hover:text-[var(--syn-accent)] truncate max-w-full">{o.title}</button><div className="text-xs truncate max-w-sm" style={{ color: 'var(--syn-text-faint)' }}>{o.description}</div></div></div></TableCell>
                        <TableCell><CategoryBadge category={o.category} /></TableCell>
                        ...
                        <TableCell className="overflow-hidden py-1" onClick={e => e.stopPropagation()}><div className={SOURCE_CELL_CLASS}>{(() => { const reference = resolveMeetingReference(o.meetingSource, o.meetingId); return <SourceChip meeting={reference?.meeting || null} deleted={reference?.deleted} onClick={() => openMeetingReference(reference)} /> })()}</div></TableCell>
                        <TableCell className={TABLE_ACTION_CELL_CLASS} onClick={e => e.stopPropagation()}><div className="flex gap-1.5 items-center justify-center"><button onClick={() => setEditOpen({...o})} className="text-base w-7 h-7 flex items-center justify-center rounded hover:bg-[var(--syn-hover)] hover:text-[var(--syn-accent)] transition-colors" style={{ color: 'var(--syn-text-faint)' }}>{'✎'}</button><button onClick={() => setConfirmDelete({ label: o.title, action: () => handleDeleteOpen(o) })} className="text-base w-7 h-7 flex items-center justify-center rounded hover:bg-[var(--syn-hover)] hover:text-[var(--syn-danger)] transition-colors" style={{ color: 'var(--syn-text-faint)' }}>{'✕'}</button><input type="checkbox" className={`w-3.5 h-3.5 cursor-pointer transition-opacity block ${openSelected.has(o.id) ? 'opacity-100' : 'opacity-0 group-hover:opacity-60'}`} style={{ accentColor: 'var(--syn-accent)' }} checked={openSelected.has(o.id)} onClick={e => e.stopPropagation()} onChange={() => setOpenSelected(prev => { const n = new Set(prev); n.has(o.id) ? n.delete(o.id) : n.add(o.id); return n })} /></div></TableCell>
                      </TableRow>
                    ))}
