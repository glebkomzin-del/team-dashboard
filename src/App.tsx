  ...
  const renderInboxSignalDetails = (entry: any) => {
    ...
    return (
      <>
        ...
        <div className="rounded-lg border border-[var(--syn-line)] p-3 space-y-3" style={{ background: 'var(--syn-surface-2)' }}>
          <div className="flex flex-wrap gap-2">
            {signal.duplicateOf && <Badge variant="outline" className="text-[10px] border-[var(--syn-danger)]/40 text-[var(--syn-danger)]">Dublette</Badge>}
            {signal.statusSuggestion?.kind === 'done' && <Badge className="text-[10px] bg-[var(--syn-ok-soft)] text-[var(--syn-ok)]">Erledigt</Badge>}
          </div>
          {signal.duplicateOf && (
            <div className="space-y-1">
              <div className="text-xs font-medium" style={{ color: 'var(--syn-text-faint)' }}>Dublette von</div>
              <button onClick={() => duplicateEntityType && openSourceEntity(duplicateEntityType, signal.duplicateOf.id)} className="text-sm text-left font-medium hover:text-[var(--syn-accent)]">{signal.duplicateOf.title || signal.duplicateOf.id}</button>
              {signal.duplicateOf.reason && <p className="text-sm leading-relaxed whitespace-pre-wrap">{signal.duplicateOf.reason}</p>}
            </div>
          )}
          {signal.statusSuggestion?.kind === 'done' && (
            <div className="space-y-2">
              <div className="text-xs font-medium" style={{ color: 'var(--syn-text-faint)' }}>Warum als erledigt markiert?</div>
              {(signal.statusSuggestion.reason || signal.statusSuggestion.resolution_reason) && <p className="text-sm leading-relaxed whitespace-pre-wrap">{signal.statusSuggestion.reason || signal.statusSuggestion.resolution_reason}</p>}
              {signal.statusSuggestion.evidence_quote && <blockquote className="text-sm leading-relaxed border-l-2 border-[var(--syn-line)] pl-3 italic" style={{ color: 'var(--syn-text-muted)' }}>{signal.statusSuggestion.evidence_quote}</blockquote>}
